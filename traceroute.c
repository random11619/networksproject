...
nleft =
2; }
if (nleft == 1){ *(unsigned char *) (&ans) = *(unsigned char *) w;
sum += ans; }
...
char dest_ip[20], recv_buffer[BUFSIZ], send_buffer[(sizeof(struct
...
strcpy(dest_ip,inet_ntoa(*(struct in_addr *)host>
h_addr));
ip >
ihl = 5;
ip >
...
tos = 0;
ip >
tot_len = sizeof(struct icmphdr)+sizeof(struct iphdr);
ip >
id = htons(0);
ip >
frag_off = 0;
...
ip >
check = in_cksum((unsigned short *)ip, sizeof(struct iphdr));
icmp >
type = ICMP_ECHO;
icmp >
code = 0;
...
checksum = 0;
icmp >
checksum = in_cksum((unsigned short*)icmp,sizeof(struct icmphdr));
...
printf("Tracing: %s (%s)\n\n",argv[1],dest_ip);
do{
bytes = sendto(sock,packet,ip >
tot_len,0,(struct
sockaddr*)&target,sizeof(struct sockaddr));
if (bytes == 1)
{
perror("send");
exit(EXIT_FAILURE);
...
{ perror("recv");
exit(EXIT_FAILURE); }
address.s_addr = re_ip >
saddr;
printf("%3d Packet
returned by: %s ",
ip >
ttl,inet_ntoa(address));
if (re_icmp >
type == ICMP_TIMXCEED)
